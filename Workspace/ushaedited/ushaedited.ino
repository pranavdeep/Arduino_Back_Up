...
else if( (a==LOW || b==LOW) && (d==LOW || e==LOW) && (g==LOW || h==LOW)  )//logic gates i.e all ands b/w a,b,d,e,g,h
{
  ...
  if( (digitalRead(ss1)==LOW && digitalRead(ss2)==LOW) && ( digitalRead(ss4)==LOW && digitalRead(ss5)==LOW ) && ( digitalRead(ss7)==LOW && digitalRead(ss8)==LOW ) )//logic gatez
  {
  Serial.println("3");
  digitalWrite(lm1,HIGH);
  digitalWrite(lm2,LOW);
  digitalWrite(rm1,HIGH);
  digitalWrite(rm2,LOW);
  delay(140);//half of the distance
...
else if((a==LOW || b==LOW) && (e==LOW || d==LOW ) && (h==HIGH || g==HIGH)  )//logic gates
{
    Serial.println("5");
  digitalWrite(lm1,HIGH);
  digitalWrite(lm2,LOW);
...
else if( a==HIGH && (g==LOW || h==LOW) && (e==LOW || d==LOW))
{
  ...
  if ( ( digitalRead(ss4)==LOW || digitalRead(ss5)==LOW ) && ( digitalRead(ss8)==HIGH ) && (digitalRead(ss1)==HIGH ) ) //ss1-ss8 are high
  {
  ...
  digitalWrite(lm2,LOW);
  digitalWrite(rm1,HIGH);
  digitalWrite(rm2,LOW);// ee part ledu
  delay(30);
       rightturn();
    }
...
void leftturn()
{
  ...
  while(digitalRead(ss1)==HIGH)
  {
    ...
    if(digitalRead(ss5)==LOW)
    {
      ...
      analogWrite(lm1, 255);//changed from 180 to 255
      digitalWrite(lm2, LOW);
      digitalWrite(rm1, LOW);
      analogWrite(rm2, 255);
