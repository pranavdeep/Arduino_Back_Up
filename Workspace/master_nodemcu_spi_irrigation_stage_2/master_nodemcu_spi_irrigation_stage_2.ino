...
void loop()
{
    ...
    for(i=0;i<3;i++)
    {
    spi_dat[i] = SPI.transfer(0x00);//Received the processed data byte from the slave
    }
    digitalWrite(CS, HIGH);                                    //Pull CS Line High
    Serial.println("Processed Data0 Recieved from Slave is: ");
    Serial.print(spi_dat[0]);    //UART - Print the data received from the slave
    Serial.println("\r\n");
    Serial.println("Processed Data1 Recieved from Slave is: ");
    Serial.print(spi_dat[1]);    //UART - Print the data received from the slave
    Serial.println("\r\n");
    Serial.println("Processed Data2 Recieved from Slave is: ");
    Serial.print(spi_dat[2]);    //UART - Print the data received from the slave
    Serial.println("\r\n");
