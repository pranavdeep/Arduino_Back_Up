...
// just to verify if sensors have read correctly or not
  ...
  digitalWrite(rm1,HIGH);
  digitalWrite(rm2,LOW);
  delay(180);////half of the  distance forward(extra inch forward movement)  //changing from 20 to 150
...
void leftturn(){
  while(digitalRead(ss1)==HIGH){
    ...
    digitalWrite(lm1, LOW);
    digitalWrite(lm2, HIGH);//earlier it was analogWrite(180)
    digitalWrite(rm1, HIGH);//changed to HIGH
    digitalWrite(rm2, LOW);
    delay(1);
    /*digitalWrite(lm1, LOW);
    digitalWrite(lm2, LOW);
    ...
    if(digitalRead(ss5)==LOW){
      ...
      digitalWrite(lm2, LOW);
      digitalWrite(rm1, LOW);
      digitalWrite(rm2, HIGH);//changing from 150 to HIGH
      delay(2);
