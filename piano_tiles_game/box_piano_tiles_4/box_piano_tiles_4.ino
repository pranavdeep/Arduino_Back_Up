...
int flag2,flag1,flag3,flag4=0;//counter variable to check if a particular level is completed or not
long total=0;
int q,r,s,t,t1=0;//variables to generate randomised cells
int check1,check2,check3=0;//variable to control the entry of compiler into randomised number generator function only once for a particular level

int l[12];//this variable/array stops the repeated glowing of led after its tapped once.
...
void level4()
{
  ...
  random_generate_3();//function generates random q,r,s values
  check3=1;
  //q=3;r=4;s=5;
...
digitalWrite(b[t],HIGH);
 ...
 if(l[t1]<1)
{
digitalWrite(g[t1],HIGH);
digitalWrite(b[t1],HIGH);
 }
 ...
 l[q]++;
  ...
  if(digitalRead(ir[t1])==0)
  {
    if(l[t1]<1)
    {
    digitalWrite(g[t1],LOW);
    digitalWrite(b[t1],LOW);
    score[t1]+=2;
    l[t1]++;
